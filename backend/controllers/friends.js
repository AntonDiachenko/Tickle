   ...
   try {
      ...
      // console.log("User ", req.body.approvedDate);
        ...
        const newFriendship = new Friendships({
            user:currentUser._id, friend:friendReq._id, approvedDate:approval,
           });
  ...
  // const list = await Promise.all(
   ...
   if(list.length !== 0){
    ...
    for(let i=0; i<list.length; i++ ){
       ...
       if((list[i].user == currentUser.id && list[i].friend==friendReq.id)
      ...
      await friendReq.updateOne({$push: { friendships: newFriendship._id }});
      await currentUser.updateOne({$push: { friendships: newFriendship._id }});
